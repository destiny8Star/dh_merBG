<template>
    <div>
        <div class="main-right" id="main-right" :style="{height: (getWinHeight-60) + 'px'}">
            <div class="third-nav">
                <div class="third-nav-t">创建折扣券</div>
            </div>
                        ...
                        <div class="form-list">
                            <el-form label-position="left" ref="form" :model="form" label-width="160px" :rules="rules">
                                ...
                                <el-form-item label="优惠内容" prop="content">
                                    <div class="ly-flex-style">
                                        <el-input v-model="form.content.discount"></el-input>
                                        <p>折，最高优惠</p>
                                        <el-input v-model="form.content.maxValue"></el-input>
                                        <p>元</p>
                                    </div>
                                    <p class="ly_hint_text">注：折扣率取值范围1-9.9，保留一位小数；金额保留两位小数，不填则不限制</p>
                                </el-form-item>
                                <el-form-item label="使用门槛" prop="min_pay_price">
                                    <div class="ly-flex-style">
                                        <p>订单满</p>
                                        <el-input v-model="form.min_pay_price"></el-input>
                                        <p>元</p>
                                    </div>
                                    <p class="ly_hint_text">注：不填或填0表示无门槛</p>
                                </el-form-item>
    ...
    export default {
        ...
        data: function () {
            ...
            var checkObjNullOne = (rule, value, callback) => {
                if(value.maxValue.trim()!=''){
                    if (!reg3.test(value.discount)||!reg1.test(value.maxValue)) {
                        callback(new Error(rule.message));
                    }  else {
                        callback();
                    }
                } else {
                    if (!reg3.test(value.discount)) {
                        callback(new Error(rule.message));
                    } else {
                        callback();
                    }
            ...
            return {
                ...
                form: {
                    ...
                    use_stime: "",//开始时间【数据联调】
                    use_etime: "",//结束时间【数据联调】
                    is_all_store: '0',//所有or多选门店【1：所有，0：选择】
                    ...
                    discount: '',//优惠内容 - 折扣【数据联调】
                    max_discount_price: '',//优惠内容 - 最高优惠【数据联调】
                    min_pay_price: '',//使用门槛【数据联调】
            ...
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        this.form.discount = this.form.content.discount;
                        this.form.max_discount_price = this.form.content.maxValue;
                        ...
                        this.$confirm('此操作将创建该卡券，是否继续?', '提示', {
                            ...
                            this.$axios.post('/mer/coupon/add/discount', this.form).then(res => {
