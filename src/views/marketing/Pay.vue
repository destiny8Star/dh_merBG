<template>
    <div>
        <div class="main-right" id="main-right" :style="{height: (getWinHeight-60) + 'px'}">
            <div class="third-nav">
                <div class="third-nav-t">会员支付记录</div>
            </div>
            <div class="main-content" id="main-content" :style="{height: (getWinHeight-120)+'px'}">
                <div class="main-feature">
                    <div class="main-inner">
                        <p style="color: red;margin-top: 0">说明：因充值金额已在充值当天入账，故会员余额支付金额不计算在当日实际到账金额中</p>
                        ...
                        <el-form :inline="true" :model="postData" class="demo-form-inline" style="margin-top: 20px">
                            <el-form-item label="支付时间" class="bill-collect-date">
                        ...
                        <template>
                            <el-table ref="multipleTable" :data="tableData" style="width: 100%;min-width: 780px;"
                                ...
                                <el-table-column prop="real_price" label="实收金额（元）">
                                    <template slot-scope="scope">
                                        {{scope.row.real_price ? scope.row.real_price : '--'}}
                                    </template>
                                </el-table-column>
                                <el-table-column prop="discount_price" label="优惠金额（元）">
                                    <template slot-scope="scope">
                                        {{scope.row.discount_price ? scope.row.discount_price : '--'}}
                                    </template>
                                </el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                //初始化列表
                this.$axios.post('/mer/vip/order/pay/listOrder', this.postData).then(res => {
                    console.log(res);
                    if (res.data.code === this.$webConfig.httpSuccessStatus) {
                        this.tableData = res.data.data.data;
                        this.tableData.forEach(ele => {
                            ele.time = new Date(ele.time).format('yyyy-MM-dd hh:mm:ss');
                        });
            ...
            getTopData() {
                //初始化列表
                this.$axios.get('/mer/vip/order/pay/topTotalData').then(res => {
                    console.log(res);
                    if (res.data.code === this.$webConfig.httpSuccessStatus) {
